...
'DESCRIPCION'	= (select case
					when ic_concepto like 'CHEQUES' 
					and ic_estado like 'V' 
					and ic_impuesto = @i_c_impuesto 
					and ic_fecha_valor between @w_f_desde 
					and @w_f_hasta
					   then 'CFJ | ' + count(ic_alicuota) + '| IFJ | ' + sum(ic_importe_comprobante)
					end,
...
'SECUENCIAL' 	= ic_secuencial,
'MONEDA'		= ic_moneda,

/* DEFINIR CON JENU COMO CALCULARLO
'CANT_FJ' = when ic_concepto like 'CHEQUES' then 
			update set 
'IMP_FJ' = '000',*/
...
and isnull(ic_cargo_banco,'N')   =  isnull(@i_m_cargo_banco,isnull(ic_cargo_banco,'N'))
group by ic_ente, ic_fecha_valor, ic_modulo, ic_cuenta, ic_sucursal, ic_alicuota
order by ic_ente, ic_fecha_valor, ic_modulo, ic_cuenta, ic_sucursal, ic_alicuota

insert into bc_sello_salida (
ss_fecha_informac,
ss_impuesto,
ss_disenio,
ss_ente,
...
case
   when ic_concepto like 'CHEQUES' or ic_monto_base_imponible = 32
   then ALICUOTA = 100 and ss_base_calculo = 1
   else ss_alicuota
end,
...
case when IMP_RET < 0 then 'E' else 'P' end + '|' +
      ltrim(rtrim(CUENTA)) + '|' + ltrim(rtrim(DESCRIPCION)), 
      convert(varchar(10), CANT),                             
      getdate() 
--ss_datos_adicionales
'DESCRIPCION',			--ss_descripcion
null,					--ss_identity
null,					--ss_fecha_registro
