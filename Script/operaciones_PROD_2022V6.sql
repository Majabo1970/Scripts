...
WHERE  1=1
    ...
    from PROD..VW_OPERACIONES_ALL o4 WITH(NOLOCK)  where o4.EspPpal =o.EspPpal and 
     Tipo in ('TrfSal','TrfEnt','Ext') and not(o4.Cliente like 'CARTERA ITAU%')))
...
set Tipo = (case Tipo   when 'Cmp' then 'Vnta'
                                   when 'Vnta' then 'Cmp' else Tipo end)
...
from #tmp a inner Join  PROD..VW_OPERACIONES_ALL b WITH(NOLOCK)  on a.MinManual=b.MinManual 
...
from #tmp p where not Tipo in ('Cmp','Vnta')
order by Tr,FchOrg , MinManual
...
from #tmp a inner Join  PROD..VW_OPERACIONES_ALL b on a.MinManual=b.MinManual 
 where a.Tipo in ('Cmp','Vnta')
...
select  p.Tr,
        ...
        O.Comentario
from #tmp p
INNER JOIN PROD..VW_OPERACIONES_ALL O WITH(NOLOCK)  ON
      O.MinManual = p.MinManual
left outer join  PROD..VW_MOVFON_ALL m WITH(NOLOCK)  on
p.MinManual = m.MinManual
  and p.Tr=m.Abrev
  and m.TipoCod = 'CB'
  left outer join  PROD..VW_MOVFON_ALL n WITH(NOLOCK)  on
p.MinManual = n.MinManual
  and p.Tr=n.Abrev
  and n.TipoCod = 'PG'
